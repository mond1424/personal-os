...
| GET `/api/guard/events?limit` | — | 발동 이력 rows + **`later_fires`**(사실) + **`outcome_inferred`**(`"failure"\|null` · 뜻). ★ **거르지 않는다** — 묻는 큐가 추론된 줄을 안 주므로(아래 `pending-outcome`) **그 줄을 보고 고치는 자리가 여기**다(T-79 ③). ⚠️ 뜻은 `outcomeInferred` 하나가 진다 — `pending-outcome`과 같은 함수 | `guard.events` |
...
| GET `/api/collected/pending` | — | `[{id, source, summary, starts_at, categories}]` · **`state='new'`이고 `starts_at`이 `[t.now, +7일]`인 것만**(T-42 결정 ①). 창 밖·과거·`dismissed`·`starts_at IS NULL`은 안 준다. **`description`은 안 싣는다** — 카드가 원문 한 줄만 쓴다. ★ **`categories`(0024 · T-74)는 싣는다 — 과목을 아는 유일한 칸이다**(`summary`는 교수가 짓는 이름이라 틀린다). **원문 그대로 나간다** — 괄호 안(학기·코드)을 떼는 것은 해석이고 **화면이 한다** | `collected.pending` |
