...
**상태** ✅ **실측 완료 (2026-08-17 낮)** · 최종 검토 합격 (수정 없음) 2026-08-11 · ⚠️ **티켓 전제 정정 있음** (§보고 첫 절)
실측: ① **`cap`이 이름을 갖고 남았다** — 그게 없었으면 *"네트워크가 없었나"*로 오진했다(이 티켓의 첫 실물 성과).
② `intervene_sec`·`screen_on_sec`이 짝으로 나왔다(41 / 39). **`network`·`timeout`의 구별은 여전히 밤이 답한다.**
**순서가 강제된다**: `0016 --remote` → deploy → APK. 확인법은 `CLAUDE.md` §사람이 하는 것의 상태.
...
**최종 검토 · 2026-08-11 · 합격 (수정 없음) · 범위 밖 둘 승인**

```
접두사가 유지되는가 (과거 행과 같은 부류로 읽히는가):
  ✅ **질문이 무의미해졌고, 그것이 이 티켓의 성과다.** 값의 모양을 안 바꿨다 —
     `ai_verdict`는 글자 그대로 `'unavailable'`이고 이유는 `ai_unavailable_reason`(0016)에 있다.
     접두사를 유지한 게 아니라 **접두사를 만들 필요가 없어졌다.** 과거 행은 그 칼럼이 NULL —
     *"모른다"*로 읽히지 *"이유가 없었다"*로 읽히지 않는다.
캐시 조건이 무변경인가 · 양성 대조가 있는가:
  ✅ `db/index.ts:659·662` 둘 다 `IN ('approve','deny')` 그대로다.
     ★양성 대조가 **감도를 갖는 모양으로** 짜여 있다 — `approve`를 깔고 그 위에 최신
     `unavailable`을 얹는다. `IN`이 살아 있으면 level 4, 지우면 level 3이다.
     담당이 **자기 검사를 스스로 떨어뜨려** 이 자리를 찾았다: 처음 `deny`로 깔았더니
     `IN`을 통째로 지워도 293/0이었다(`guard.ts:392`가 `=== "approve"`만 보므로 결과가 같다).
     그 발견이 `AGENT-CHAIN.md` §5의 *"대조는 결과가 달라지는 값으로 짠다"*가 됐다.
screen_on_sec 이 그대로인가 (더하고 빼지 않았는가):
  ✅ `GuardActivityLog.kt:185·187` — `screen_on_sec` 그대로 두고 `intervene_sec`을 **옆에** 놓는다.
     감산 자리가 없는 것을 smoke가 소스에서 확인한다(짝: 새 항만 보면 기존 항을 지워도 통과한다).
     **창 필터보다 앞에서 읽는 판단이 맞다** — 개입은 창 밖에서 시작해 창 안으로 이어지고
     (소리는 3분에 멎지만 화면은 `KEEP_SCREEN_ON`으로 남는다) **그게 T-30이 잡은 그 밤이다.**
이유가 닫힌 목록인가:
  ✅ 대장 하나(`UNAVAILABLE_REASONS` 여덟 + `http_NNN`) · 0016 CHECK · `GuardVerify.kt object Reason`
     **셋이 일치하고, 갈라지면 smoke가 빨간불이다**(스캐너 기대값이 비어 있지 않아 스캐너가
     죽어도 빨간불 — T-26의 교훈 자리).
     ★**목록 밖은 이유만 비우고 행은 살린다.** 400을 던지면 CHECK 위반과 결과가 같아져
     `flush()`가 발동 행을 버린다 — **이 티켓이 늘리려는 바로 그 행이다.**
     구버전 서버 + 신버전 APK가 실제로 그 자리다.
설계 위반 · 함정 재발:
  없음. 파생 물화 0(빼지 않고 더했다) · 캐시·발동·격상 순서 무변경 ·
  **함정 7**(새 마이그레이션 → `test/smoke.ts` 스키마 목록)에 `0016_guard_unavailable_reason.sql`
  들어가 있다 · `docs/schema-current.sql` 재덤프됨(칼럼과 CHECK 둘 다 반영) ·
  `ADD COLUMN` 한 줄이라 **개입 이력 테이블을 재작성하지 않는다**(영구 보존 원장).
판정:
  ✅ 합격 — 수정 없음.
```
...
```
GuardNotifications.kt   verify() → attempt()   이유를 받아 amendFire에 넘긴다
GuardEventQueue.kt      amendFire(unavailableReason = null)
```

**AGENT-CHAIN §5 '티켓이 틀렸을 때'의 세 조건을 다 만족한다.**
① 티켓의 목적이 *"이유를 기록에 남긴다"*이고 ② 이유를 **싣는 자리**는 `GuardNotifications.kt:246`,
페이로드를 만드는 자리는 `amendFire`다 — 셋을 잇지 않으면 이유가 계산만 되고 **아무 데도 안 남는다**.
③ §설계와 어긋난 점에 무엇을 왜 넓혔는지 적혀 있다.

**§범위가 `GuardVerify.kt`만 적은 것이 발행자의 누락이다** — *"만드는 곳"*과 *"싣는 곳"*을 구별하지
않았다. `AGENT-CHAIN.md` §5의 *"여는 자리와 붙는 자리를 구별한다"*가 여기에도 그대로 걸린다.
**발동·격상 순서는 무변경이다** — `attempt()`가 `verify()`가 하던 일을 그대로 하고
`verify()`는 얇은 겉면으로 남았다. fail-closed도 6초도 캐시 조건도 그대로다.

### 남은 것 — 이 층이 아니라 밤이 답한다

- **Kotlin 러너가 없다.** `Reason`·`snapshot()`은 smoke가 **소스를 읽어** 확인한 것이지 실행이 아니다.
  실검사는 `docs/FIELD-TEST-DAY.md` ②(개입 초).
- **`unavailable`의 이유는 다음 밤이 답한다.** 비행기 모드는 `timeout`이 아니라 즉시 실패라
  새벽 조건과 다르다 — **흉내로 결론 내지 않는다**는 티켓의 판단이 맞다.
- ⚠️ **`network`가 뭉뚱그려질 것이다.** 실측이 답하기 전에 목록을 늘리지 않는 판단도 맞다 —
  늘리는 일이 0016 CHECK · TS 대장 · Kotlin 셋을 함께 고치는 일이기 때문이다.
