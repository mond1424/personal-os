...
object GuardNotifications {
    ...
    fun fire(
        ...
        /*
         * ★ **오늘 밤 사용자가 방해받은 횟수** (T-69 ① · ADR-026 *"재확인도 하나의 발동"*).
         *
         * ★ **세는 자리가 여기 하나다.** 감시·재확인·경로 A 알람이 전부 이 함수를 지나므로
         *   경로가 늘어나도 저절로 세어진다 — 경로마다 세면 다음 사람이 넷째를 더하고 잊는다
         *   (T-70의 `markAsked`를 발동 경로가 아니라 화면으로 옮긴 것과 같은 이유).
         *
         * ⚠️ **`GuardWatch.K_NIGHT_N`이 아니다.** 그쪽은 밤 상한이 읽는 **동작의 값**이고,
         *    거기에 재확인까지 더하면 감시 발동이 줄어든다(T-69 §금지 1행).
         *
         * ⚠️ **닿았을 때만 센다.** 권한이 없어 알림도 화면도 못 낸 발동은 **아무도 안 봤다** —
         *    그것을 *"방해"* 로 세면 T-70이 없앤 거짓(안 물어봐 놓고 세는 수)과 같은 모양이 된다.
         *    창 밖인지는 `GuardNight.note`가 가른다 — 문구가 창을 말하기 때문이다.
         */
        if (posted || shown) runCatching { GuardNight.note(ctx) }
