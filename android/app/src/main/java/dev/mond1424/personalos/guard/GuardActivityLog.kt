...
object GuardActivityLog {
     ...
     *
     * **기기는 점수를 내지 않는다.** 항 값만 남긴다 —
     * 위험도는 기록하되 발동 게이트로 쓰지 않는다(ADR-021).
     * 가중치는 10월에 실제 스냅샷에서 유도한다.
     *
     * **점수는 서버가 낸다** (T-32 · `services/guard.ts`의 `riskScore`). 발동이 끝난 뒤
     * `record()`에서 계산하므로 구조적으로 게이트가 될 수 없고, 그것이 ADR-021이 요구한 형태다.
     * 여기서 내면 발동 경로 안이라 그 보장이 사라진다.
     *
     * **§6.6의 서버 출처 항(Log·Feelings·Score·데드라인)은 여기 없다** — 기기엔 그 데이터가
     * 없기 때문이다. 서버가 `record()`에서 `server` 키 아래에 얹는다(T-32).
     *
