...
object GuardWatch {
    ...
    private fun evalInner(ctx: Context): Boolean {
        ...
        val body = buildString {
            ...
            /*
             * ★ **창의 끝(`bedTo`)은 문구에 없다** (T-76 ① · ADR-047 ①).
             *   한 문장 안에서 **매일 같은 것은 이 조각뿐**이었다 — 순번도 아침 한 줄도 분도
             *   밤마다 달라지는데 `00:30~06:00`만 고정이라, `windowMin`이라는 사실을 읽으려면
             *   **매일 같은 열 몇 글자를 먼저 지나야** 했다. 위 `if (wakeLine != null)` 주석이
             *   *"사실이 맨 앞"* 이라고 적은 그 경계를, 이 조각은 **가운데에 있어서** 빠져나갔다.
             * ★★ **그리고 끝은 소음보다 나빴다 — 오독을 만든다.** 사용자가 `~06:00`을 기상
             *   시각으로 읽고 바로 위의 *"8시 기상"* 과 비교해 **둘 다 틀렸다**고 판정했다.
             *   `bedTo`는 *"이 시각 이후엔 감시를 안 한다"* 는 **앱 내부 사정**이지 기상이 아니다.
             *   ⚠️ T-69가 *발동 횟수*의 두 수에서 고친 것과 **같은 결함이 시각 축에서 다시 났다.**
             * ★ **시작(`bedFrom`)은 남긴다 — 빼면 안 된다.** `windowMin`이 재는 것이 정확히
             *   `windowStartMs(bedFrom, bedTo)` 이후라, **무엇 이후의 N분인지**를 말해야 사실이
             *   완성된다. 끝은 그 사실에 아무 기여도 안 하지만 시작은 기여한다.
             * ⚠️ **`bedTo`를 코드에서 지우는 것이 아니다** — 바로 위 `windowStartMs`와 `inWindow`가
             *   창 계산에 쓴다. **문구에서만 뺀다.**
