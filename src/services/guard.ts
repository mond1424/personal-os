...
//
// **Level 4의 대가는 아직 적지 않는다** (T-28). 원래 "신규 작업을 막는다"는 뜻의 괄호가
// 붙어 있었는데 그 강제력은 구현이 없었다 — 모델이 없는 대가를 저울에 올려 판정했고,
// 그렇게 쌓인 `guard_events`가 §6.5가 읽을 전례다. ADR-035가 실제 형태를 정했지만
// (오늘에 붙는 날짜를 내일로) 아직 구현 전이므로 **거짓을 지우기만 한다.**
// 사실을 적는 것은 강제력이 생긴 뒤다(T-29) — 지금 새 문구를 적으면 거짓말을 바꾸는 것이다.
const VERIFY_SYSTEM = [
