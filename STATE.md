# STATE — 최종 갱신 2026-08-03

## 저장소
- repo: https://github.com/mond1424/personal-os
- branch: main
- 마지막 코드 커밋: `1df37a5` feat(guard): 수락 재확인 + 무음 존중 (ADR-026) — 실측 미완.
...
**Kotlin에는 검사 러너가 없다** → `:app:compileReleaseKotlin` BUILD SUCCESSFUL로 최소선만 확인했다.
**완료 판정은 실측에 걸려 있다**(티켓 §확인 절차, APK 재빌드 필요).
...
- **폰 실측(2026-07-31)**: Goals 빈 상태·추가/목록, 일반 기간 연결 시 디데이 미표시, 하단 바 Me 구분선,
  탭 스와이프는 확인 완료. **constraint 기간 연결과 실제 디데이 표시는 미확인** — 폰 화면에서 constraint 기간을
  만들거나 지정하는 진입 방법을 찾지 못했다.
...
## 미해결 / 다음 할 것
- ⚠️ **T-11 — APK 재빌드·실측 대기 (2026-08-03).** Kotlin release 컴파일과 코드 대조만 통과했다.
  - `[알겠습니다]` 뒤 계속 사용하면 5분 후 같은 Level 재확인, 밤당 2회 상한
  - 수락 뒤 화면을 끄면 재확인하지 않음
  - 무음 모드 Level 4가 소리·진동 없이 화면만 띄움
  - Me 설정에서 `overrideSilentAtL4` 제거 확인
  - 최신 `guard_events`의 `cause="recheck:accepted"` 확인 (`G.recheckStatus()`로 진단)
- ⚠️ **T-09 constraint 기간 연결·디데이 실측 대기 (2026-07-31).** 자동 검사는 통과했지만 폰 UI에서
  `kind='constraint'`·`dday_label`인 기간을 만드는/지정하는 경로가 확인되지 않았다. 연결 방법을 확인한 뒤
  목표에 선택해 실제 `라벨 D-N` 표시를 실측해야 한다.
- ⚠️ **T-04 — APK 재빌드 + 실측 대기 (2026-07-31).** 코드는 컴파일 통과, 동작은 미확인.
  티켓 §확인 절차 5줄. 특히 마지막 둘 — **`ai_used`·`ai_verdict`가 실려 오는가**(없으면 통제 ③이
  뚫린 채 배포된다) · **킬 스위치 off면 항상 Level 4 · `ai_used: 0`**.
