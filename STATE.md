...
- branch: main
- 마지막 코드 변경: T-14 adb+CDP 기기 하네스 1단계 (이 STATE와 같은 커밋).
  T-13 릴리스 서명 결과 가드는 `4390699`, T-11은 `1df37a5`, T-12 날짜 독립 검사는 `e55b515`,
  기존 T-09 두 커밋은 `6d0bb35`·`8139f7b`로 원격 이력 그대로 유지했다.
...
## 마이그레이션
최신: **`0015_me_history_reason`** — ✅ **로컬 적용 완료** (2026-08-05) · ⚠️ **원격 미적용 · deploy 대기**
- `0015_me_history_reason` (T-10) — `me_history`에 `reason TEXT` (NULL 허용). Guard 모드 하향의 사유가 앉는 자리다(ADR-027).
  **새 테이블을 만들지 않는다** — 만들면 §6.5 자기 보정이 읽을 곳이 둘로 갈린다. `ALTER TABLE ... ADD COLUMN` 하나뿐이라
  ...
  - ⚠️ **사용자가 할 것**: `npx wrangler d1 migrations apply personal-os --remote` → `npm run deploy` (배포보다 마이그레이션이 먼저).
...
## 미해결 / 다음 할 것
- ⚠️ **`0015` 원격 미적용 · deploy 대기 (T-10, 2026-08-05).** 로컬은 적용 완료.
  `--remote` 첫 시도가 Cloudflare API **7403**로 막혔는데, 같은 계정·토큰으로 `d1 list`·
  `migrations list --remote`·원격 `SELECT`가 모두 통과해 **일시적 인증 실패로 판단**했다
  (계정 ID·`database_id`·`d1 write` 스코프 전부 일치). 재시도가 답이고, 반복되면 `wrangler login`.
  **모드 하향 마찰은 배포 전까지 라이브에 없다** — 지금 폰에서 모드를 내리면 여전히 그냥 내려간다.
