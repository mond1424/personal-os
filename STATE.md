...
- 🔴 **밤 03:00 실측이 W5(8/26) 게이트다. 절차는 `docs/FIELD-TEST-NIGHT.md`.**
  `APP-PLAN` W5의 "Guard 루프가 닫혔다"에 **밤 발동이 포함된다**(2026-08-10 명시).
  낮 확인은 화면이 켜져 있어 전부 통과하지만, Guard가 일하는 시간은 **01:30~05:00이고 그때 폰은 Doze**다 —
  그리고 이 실패는 **아무 오류도 내지 않는다.** 알림이 안 뜨는 것으로만 드러나고 그때는 그 밤이 지났다.
  §완성의 정의의 03:00 알람 항목이 **7월 말부터 빈칸**이다. **자기 전 10분이면 준비된다.**
  T-26이 고친 `schedule()`이 만드는 그 예약(01:30 L3 · 02:00~ L4 6회)이 과녁이고, 이미 라이브다.
