...
- ⚠️ **Codex 환경의 `npm run front`가 세 번 연속 실패했다** (T-01 EPERM · T-03 미측정 · T-05 300초 시간 초과).
  매번 이 층이 대신 재실행해 수치를 냈다. 원인은 `xdg.config\.wrangler\logs` 권한과 격리 D1 마이그레이션
  단계로 보인다 — **Codex가 스스로 숫자를 못 내면 보고 형식의 절반이 늘 비어 온다.** 환경을 고칠 것.
