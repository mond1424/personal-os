...
- ⚠️ **서버 `stay3()`의 `timeout`이 `ai_used: 0`이다 (T-04에서 발견 · Cowork 판단).**
  타임아웃은 모델 호출이 나간 뒤일 수 있어 **돈은 나갔는데 일일 상한에 안 세어진다.**
  고친다면 `timeout`은 `ai_used: 1`이어야 한다 — 호출이 나간 사실이 상한의 대상이다.
