...
- 마지막 코드 변경: **T-24+T-25 `05b50df`** (2026-08-07). 그 앞이 T-23 `5ee0df6` · T-22 `2aa4237`.
  ⚠️ **셋 다 아직 배포 전이다** — `npm run deploy`는 사용자가 한다. 마이그레이션은 없다(0015가 마지막).
...
- ⚠️ **T-08′·T-20·T-21·T-22·T-23·T-24·T-25 폰 실측 미실시.** 화면·체감이 결과물인 것들이다.
  ...
  **T-22는 새벽 개입이 그 전날에 묶여 있는지**(`watch:bedtime` 실데이터로만 확인된다),
  ...
  ⚠️ **T-22·T-23·T-24·T-25가 전부 배포 전이다** — 마지막 배포는 08-06 04:40(T-21까지).
  마이그레이션은 없으므로 `npm run deploy` 한 번이면 넷이 함께 나간다. APK 재빌드도 불필요하다.
...
- ✅ **`ai_used` 의미 — T-07로 닫혔다 (2026-07-31).** 요청이 나갔으면 1. ⚠️ **deploy 대기.**
