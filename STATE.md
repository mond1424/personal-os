...
**1부 — `SWIPE_EDGE_RATIO` 0.1 → 0.4.** T-25의 10%가 실사용에서 좁았다(사용자 판단).
`app.js` diff는 **정확히 한 줄**이다 — `AXIS_LOCK`·`FLICK_V`·`VEL_*`·`dragBlockUntil`은 무변경.
대가를 알고 넓혔다: 달 넘기기는 **가운데 20%(7열 중 1.4열)에서만** 시작된다.

**이 티켓의 요점은 폭이 아니라 검사가 폭을 따라가느냐였다.** T-25 검사가 `+190`·`+10`·`+370`·
`37/39/341/343`을 박아 뒀는데, 폭만 바꾸고 검사를 두면 **경계값이 옛 폭을 가리킨 채 초록**이 된다.
전부 `ev("SWIPE_EDGE_RATIO")`에서 유도하도록 다시 썼고 — **검사가 자기 사본을 두지 않고 구현이
쓰는 그 변수를 읽는다** — 폭 0.25로 바꿔 **실제로 돌려 257/0**을 확인했다.
...
⚠️ **폰 실측 대기 (확인 절차 2·3번이 이번의 핵심이다)**:
**가운데에서 달이 여전히 넘어가는가**(40%의 대가) · **주와 주가 갈리는가**(3px→1px의 교환).
답답하면 30%·25%로 내린다 — 값 하나라 되돌리기 쉽다.
...
(T-27 가장자리 폭 0.4 · 캘린더 가로 구분: front 256→257 — 행 구분 1건. **제스처 검사 여섯은
