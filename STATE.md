...
- 📋 **`renderCalendar`의 `cur` 토글이 회전 경로에서 두 번 돈다**(`app.js:1053`·`1059`).
  1059가 무조건 실행되므로 1053은 잉여다. 비용은 미미하나 회전 측정에 잡음으로 들어간다.
  **고치지 않았다 — 티켓으로 낸다**(코드 소유 경계).
