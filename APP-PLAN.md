...
**2순위 — 감각 계층(Phase 8) · 입력 이관(Phase 7).** 캘린더 읽기만 8월 W5, Phase 8은 9월 1주 착수, FCM·인증은 10월. (2026-08-10 우선순위 교체: "알아서 돌아가는 구조"가 목적인 만큼 감각기관 > 표시 표면)
**위젯은 8월에 섰다(Phase 5).** 표시 표면이 감각 계층을 밀어낸 것이 아니라, **비용 전제가 실측에서 무너졌다** — 셋 다 서버를 읽지 않아 토큰·API 규격·갱신 스케줄이 통째로 빠졌다(ADR-041·043).
...
9~11월 ─ 실사용. 병렬 우선순위:
  ...
  ④ 10월: FCM (Phase 4 잔여) · 인증 골격 (Phase 0)
  ⑤ Compose 스파이크 (캘린더 1화면) — 여유 시
  ⑥ Phase 9 Obsidian 연동 — S0(맨손 1주) 판정 뒤 S1. 선행 T-85는 다음 학기 시간표 전
       ↓
...
> 이 표가 정하는 것은 **감각 계층이 표시 표면보다 앞선다**는 순서다 (2026-08-10 사용자 확정).
> 위젯이 8월에 선 것은 그 순서를 뒤집은 것이 아니라 **그것이 감각 계층의 시간을 안 썼기 때문**이다(§Phase 5).

### 8월 밖으로 보낸 것

FCM · 오버레이 · AI 발동 판단 · 자기 보정 로직 · PC 에이전트 · 인증 · PWA/오프라인 · **캘린더 쓰기 방향 · share-target · 알림 액션 · RemoteInput 마감 (Phase 7)** · **Health Connect · 메신저 캡처 (Phase 8)**.
...
ADR-007(Capacitor + FCM)은 **폐기가 아니라 시점 분리**다.
⚠️ **다만 그 수요처가 하나 사라졌다** — 위젯이 서버를 안 읽고 앱이 밀어 주므로 **위젯 갱신 푸시는 필요 없다**(§Phase 5). 남은 용도는 서버발 알림뿐이고, **그것이 실제로 필요한 순간이 아직 없다.**
...
- **결정론 규칙으로 Level 1~3 발동** (발동 규칙 명세는 Phase 6 참조).
  **Level 4는 결정론만으로 뜨지 않는다** — `GuardNotifications`가 먼저 3으로 내고,
  `GuardVerify` 승인이 있어야 격상한다(ADR-024 · fail-closed). 킬 스위치가 `off`면 결정론으로 4가 뜬다.
  *(2026-08-10 문구 정정 — 원문 "Level 1~4"는 코드와 어긋났다. 어긋난 것은 코드가 아니라 이 줄이었다)*
...
## Phase 7 — 입력 이관 (읽기 착수 · 쓰기 + 기록 경로는 9월) — 2026-08-10 신설

**읽기가 이 프로젝트에서 "넣을 것이 없다"를 처음 공략한다.** 위젯 셋(Phase 5)은
*넣기의 비용*을 깎았고, 캘린더 읽기는 *넣지 않아도 되게* 한다 —
**앱을 열어도 볼 것이 없는 상태**가 8/19~22 나흘 연속으로 관측됐고, 그것이 착수 이유다.
...
**정체성: 새 입력원 증설 — "알아서 돌아가는 구조"의 감각기관.** 근거 = ADR-032·037·034. Guard 경로와 접점이 없는 독립 모듈이라 실사용 시작과 병행 가능. **착수는 불확실성 오름차순으로 하나씩** — 앞 모듈이 실사용에서 자리 잡은 뒤 다음 모듈.
...
| 2 | **학사 일정 수집기** (ADR-037) | 기존 cron: iCal 토큰 URL fetch → `UID` diff → 제안 | **마감만.** 공지 글은 캘린더에 없다 — v2. 파서 없음(표준 iCal), 비밀번호 없음(토큰). **셋으로 나눠 착수한다: 수집(8월) → 제안 카드 → 해석(개강 후)** — 아래 함정 |
...
- **iCal 창(-5일 ~ +365일) 안에 실제 과제 이벤트가 없는 동안은 `SUMMARY`를 해석하지 않는다.**
  1학기 마감은 전부 과거고 2학기는 아직 안 열렸다. 지금 있는 표본은 **개인 일정 하나**뿐이라
  강좌 구분·`SUMMARY` 형식이 미확인이다(ADR-037 §실측).
  **그 상태로 해석하면 검사가 추측을 검사한다** — 초록인데 개강 첫날 틀리는 종류다.
  ⚠️ **막히는 것은 해석이지 수집이 아니다.** `UID`·`DTSTART`·`LAST-MODIFIED`는 구조가
  확정돼 있고, 원문을 버리지 않고 그대로 보관하면 **개강 전에 만들어도 틀릴 것이 없다**(T-41).
  그리고 **수집을 미루면 개강 첫 주가 통째로 유실된다** — 창이 좁아 다시 못 가져온다.
- *"화면에 11건이 있으니 데이터는 있다"*가 실제로 한 번 빗나갔다: 화면의 11건은
  테마가 그리는 것이라 iCal에 0건이었고, **캘린더 화면은 내보내기의 표본이 아니다.**
...
**셋이 서 있다** — `+`(할 일 추가) · 오늘 찍기 · 로그 쓰기. 전부 8월(T-46·T-48·T-49).

**"첫 위젯이 인프라 비용을 전부 떠안는다"는 전제가 실측에서 무너졌다.**

```
AppWidgetProvider 등록   들었다
다크모드 색              들었다 (values/ ↔ values-night/ 짝)
기기 토큰                ✗ GuardSync 가 이미 baseUrl+token 을 들고 있었다
/api/widget/* 규격       ✗ 화면이 쓰는 API 가 그대로 맞았다
갱신 스케줄              ✗ 위젯이 서버를 안 읽는다 — 하루 경계 알람 하나뿐
```

**셋 중 셋이 "데이터를 그릴 때만" 드는 것이었고, 그리지 않기로 하자 사라졌다**(ADR-041).
그리기로 한 뒤에도(ADR-043) 다시 들지 않았다 — **위젯은 읽지 않고 앱이 밀어 준다.**
...
|---|---|---|
| `+` | 1×1 | 딥링크 → 할 일 추가 입력창 |
| 오늘 찍기 | 리사이즈 3단계 | feelings 셋 → +요약·score → +로그 버튼 |
| 로그 쓰기 | 3×1 | 딥링크 → 로그 입력줄 |

- **문장 공장은 `public/app.js`의 `closeSummaryText` 하나.** 웹이 만든 문자열을
  `WidgetPlugin` 다리로 건넨다 — Kotlin 복제가 없으므로 앱과 위젯이 갈라지지 않는다
- **요약과 score 는 한 컨테이너다.** 한쪽만 켜는 자리가 코드에 없다(ADR-043 §결정 ②)
- **탭이 거부되면 칠한 칸이 되돌아오고 그 사실이 남는다** — 마감된 날의 409가 실제 경로다
- ⚠️ **`RemoteViews`에 `EditText`가 없다.** 위젯 안 타이핑은 Android 가 주지 않는다 — 딥링크뿐이다
- ⚠️ **`SeekBar`도 없다.** 드래그를 전달할 통로가 없어 눈금은 탭 5칸(`2·4·6·8·10`)이다
- ⚠️ **`minSdk = 24`라 `RemoteViews(Map<SizeF,…>)`(API 31+)를 못 쓴다.**
  레이아웃 하나 + `onAppWidgetOptionsChanged`의 `setViewVisibility`

### 다음을 정하는 것은 실사용이다

**한 주 동안 셋을 나눠 센다.** `+`와 `로그 쓰기`는 **같은 물건에 다른 옷**이라
그 비가 *"안 눌리는 이유가 어포던스인가"*를 가른다(ADR-043 §재검토).
거의 안 눌리면 **위젯 계열 전체를 접는 것**도 후보이고, 그때는 ADR-031(알림 액션)이 다음이다.
...
설계의 정본은 [`docs/pos-obsidian-design.md`](./docs/pos-obsidian-design.md)다. 단계(S0~S3)와 완료 기준은 그 문서 §12를 따르고, 이 절은 pOS 쪽 작업의 순서만 둔다.

| 순서 | pOS 쪽 작업 | 설계 근거 | 시작 조건 |
|---|---|---|---|
| 1 | T-85 시간표 학기 누적 · 과목명 NFC | DEC-27 · DEC-30 | 지금. 다음 학기 시간표를 넣기 전에 배포 |
| 2 | day 렌더러 · 내보내기 API(manifest · files) · 수신 시각 상태 줄 | §5.1~5.5 · §6.1 · S1 | P-7 판정(DEC-29)이 S1 진행이고, OPEN-1·2·3·5·17이 정해짐 |
| 3 | 정본 과목 목록(모든 학기) · 과목 대응표 · task·course·period 투영 · day 링크(`pos_schema` 2) | §5.4 · §5.5 · §6.1 · S2 | S1 완료 기준 통과 |
| 4 | 필기 경로 상태를 pOS 상태 줄에 | OPEN-14 | S2에서 두기로 정했을 때 |
| 5 | Me 이전 · 탭 축소 · 지난 분석 투영 | P-2 · DEC-16 · S3 | S1·S2 완료 기준 통과 · ADR |

- 2 이후의 티켓은 P-7 판정 뒤에 발행한다.
- Obsidian 플러그인과 필기 투영기는 pOS 밖의 구성 요소다. 코드 자리는 OPEN-3에서 정한다.
- pOS 앱 화면에서 S1·S2에 바뀌는 것은 설정의 상태 줄뿐이다. 탭 구성은 S3에서 바꾼다(DEC-16).

---

## 6. 9~11월 — 실사용 기간

**본체는 Guard 규칙의 반복 수정이다.** 나머지는 §3 병렬 우선순위를 따른다 (① 감각 계층 ② 입력 이관 잔여 ③ Guard 잔여 ④ 10월: FCM·인증 ⑤ Compose 스파이크 ⑥ Obsidian 연동).
...
| 위젯으로 찍은 수 / 앱으로 찍은 수 | 접근성이 원인이었나 — 아니면 위젯 계열을 접는다 (ADR-043) |
| `+` 와 `로그 쓰기` 의 비 | 같은 물건에 다른 옷이다. 갈리면 원인은 **어포던스** |
...
### 스파이크가 낸 판단 입력 (2026-09-14 · T-73 · 캘린더 한 화면 · 사용자 왕복 3회)

**★★ 비용이 한 방향이 아니다. 둘이 서로 반대로 잡혔고, 그것이 이 스파이크의 결론이다.**

```
★ 이득   제스처 상수는 옮길 값이 아니었다
⚠️ 비용   제스처가 겹치는 자리는 새 결함 부류를 만든다
```

#### ★ 이득 — 여덟 숫자 중 하나는 제품이 아니라 브라우저 보정값이었다

**§6이 *"이 숫자들이 제품이다"* 라고 한 것은 전부에는 해당하지 않는다.**
사용자가 두 팔 스위치로 직접 비교했다:

```
touchSlop  8.2dp (플랫폼)  ★ 훨씬 낫다
           20dp  (AXIS_LOCK) ⚠️ 옮기면 더 나빠진다
```

**`AXIS_LOCK 20`은 브라우저 포인터 이벤트의 노이즈를 견디려고 크게 잡은 값이고,
네이티브엔 그 노이즈가 없으니 보정도 필요 없다.** 그래서 **"튜닝 재유도"를 비용으로만 적지 않는다.**
⚠️ **다만 만져 본 것은 슬롭 하나뿐이다** — 축비·속도 임계는 판정되지 않았다. 일반화하지 않는다.

#### ★★ 단위 — 무엇이 넘어가는지가 칸 셋으로 갈린다

```
레이아웃   ★ 넘어간다              1 CSS px = 1 dp. gap·반지름·행 높이
제스처     안 넘어간다              재는 주체가 통째로 바뀐다
★ 텍스트   레이아웃인데 안 넘어간다  줄 상자를 폰트가 정하고 폰트는 플랫폼 소유다
```

⚠️ **셋째 칸을 빼면 전환 비용이 화면 수만큼 과소평가된다** — 글자 있는 모든 화면에 걸린다.
★ 고치는 법은 오프셋이 아니라 **줄 상자를 선언하는 것**(`lineHeight` + `LineHeightStyle`)이고,
그러면 폰트·배율이 바뀌어도 안 깨진다. **합격선을 dp로 적으면 안 된다** — 플랫폼이 정수 픽셀로
양자화하므로 *"정확히 N dp"* 는 어떤 폰트에서도 안 나온다.

#### ⚠️ 비용 — 제스처가 겹치는 자리에 **웹엔 없던 결함**이 생긴다

**웹은 `pointerdown`에서 동기로 답하고, 기준 프레임이 고정 컨테이너다 — 답이 항상 정의돼 있다.**
Compose 는 그렇지 않다:

```
shouldAwaitTouchSlop = !startDragImmediately()
startDragImmediately() = isScrollInProgress || overscrollEffect.isInProgress
```

**전이 중에는 슬롭을 안 기다린다**(스크롤 중인 목록을 잡아채는 그 동작 — 의도된 정상 동작이다).
그래서 *"안쪽이냐 바깥쪽이냐"* 를 가르는 게이트가 **반영될 틈이 0**이다.
실제 증상: **탭 전환이 끝나기 전에 가운데를 끌면 목표 탭을 지나쳐 계속 간다.**

★★★ **이것은 튜닝 문제가 아니다:**

```
지연이라면   틈이 짧다        →  더 큰 슬롭·더 이른 패스로 늘린다  = 튜닝
실제로는     플랫폼이 없앤다   →  늘릴 것이 없다                   = 모델
```

⚠️ **그리고 비용이 화면 수가 아니라 *제스처가 겹치는 자리 수*만큼 난다** —
웹 소스 실측 `noSwipe 13` · `noStretch 8`, 그리고 **탭 다섯이 전부 가로 스와이프**다.
★ `overscrollEffect.isInProgress`가 같은 문을 열므로 경계 스트레치 복귀 중에도 같은 구멍이 난다.

⚠️ **이 진단은 바이트코드 정적 확인이고 관측과 정합하지만, 기기 계측으로 재현한 것은 아니다.**

#### 집계

```
해결 7 · 우회 3 · 못 함 2 · ★ 구조 1 · 안 쟀다 3
왕복 3회 — 제스처 1 · 렌더 1 · 성능 1 · ★ 구조 1
```

★ **"못 함 2"(축비·속도 임계)의 무게는 위 §이득이 덜어낸다** — 없는 것이 결함이 아니라
플랫폼이 같은 문제를 다르게 푼 것이고, **적어도 한 건은 사용자가 그쪽을 더 좋아했다.**
⚠️ **"구조 1"은 반대다. 그 하나가 다른 칸 전부보다 무겁게 읽힌다.**

### ⚠️★★★★ 정정 — **"캘린더가 느리다"는 이 결정의 입력이 아니다** (2026-09-22 · T-81 ②)

**사용자가 *"캘린더 스크롤 딜레이 길어서 스파이크 쓰고 싶다"* 고 말했고, 그것이
전환을 재촉하는 근거로 읽혔다. 재 보니 아니었다.**

```
느린 곳   ★ 한 칸 넘길 때 두 칸 뒤의 달을 API 로 기다린다 (왕복 바닥 0.48~0.55s + D1 여섯)
렌더      ✗ pane 하나 110~320 노드. 그 옆에서 잴 크기가 아니다
전환 시간  △ 300ms 고정. 주범이 아니다
프레임    ⚠️ 셋으로 설명이 된다 — 후보가 아니다
```

★★★★★ **네이티브로 옮겨도 이 왕복은 그대로다.** 고칠 자리는 웹이었고,
**T-82가 *기다리는 대상*을 바꿔서 고쳤다**(`calBusy` 를 화면 전환이 끝나면 푼다).

⚠️⚠️ **그러므로 위 §판단 입력의 *"9~11월에 실제로 매일 쓴 화면"* 에 이 불편을 넣지 않는다** —
**넣으면 이미 웹에서 고친 것을 전환의 이유로 두 번 세게 된다.**
★ **전환이 필요한 이유는 따로 생겨야 하고**, 지금 그 자리는 **"안 쟀다 3"** 뿐이다.

★★ **같은 모양이 T-73 왕복 1에서 한 번 났다**(*"Compose 에 슬롭을 넣을 자리가 없다"* 가
바이트코드로 뒤집혔다 — 함정 18). **증상을 원인으로 읽으면 전환 비용 추정이 양쪽으로 틀린다.**
