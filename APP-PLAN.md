...
| 2 | **학사 일정 수집기** (ADR-037) | 기존 cron: iCal 토큰 URL fetch → `UID` diff → 제안 | **마감만.** 공지 글은 캘린더에 없다 — v2. 파서 없음(표준 iCal), 비밀번호 없음(토큰). **개강 첫 과제 이벤트 전에는 착수하지 않는다** — 아래 함정 |
...
- **iCal 창(-5일 ~ +365일) 안에 실제 과제 이벤트가 없는 동안은 2번을 착수하지 않는다.**
  1학기 마감은 전부 과거고 2학기는 아직 안 열렸다. 지금 있는 표본은 **개인 일정 하나**뿐이라
  강좌 구분·`SUMMARY` 형식·`DTSTART`의 뜻이 전부 미확인이다(ADR-037 §실측).
  **그 상태로 만들면 검사가 추측을 검사한다** — 초록인데 개강 첫날 틀리는 종류다.
