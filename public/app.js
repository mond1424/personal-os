...
function closeSummaryText(T) {
  ...
  // Guard 조각은 **뒤에** 붙는다 (T-45) — 앞에 두면 Guard가 주인공이 되고, 이 앱은 Guard 앱이 아니다.
  // ★ 조각이 던져도 **앞 문장은 살아남는다.** 기록의 봉인이 우선이고(§금지 8행),
  //   여기서 삼키지 않으면 사용자가 넣은 것까지 같이 사라진다.
  let tail = "";
  try { tail = guardFragment(T && T.guard); }
  catch (e) { tail = ""; console.warn("Guard 조각 실패 — 앞 문장과 마감은 그대로", e); }
  return tail ? `${s} ${tail}` : s;
}

/* ── Guard 조각 — 시스템이 한 일 (T-45) ────────────────────────────────
 * `done`·`todo`·`reassign`은 **사용자가 넣어야 생긴다.** 넣은 게 0인 날에는 T-44의 요약도
 * *"담긴 할 일이 없는 날"*밖에 못 말한다 — 그런데 그 나흘에도 Guard는 매일 무언가를 했다.
 *
 * ★ **주어는 시스템이다.** 같은 사실을 *"새벽 2시까지 깨어 있었어요"*로 쓰면
 *   평가어가 하나도 없어도 **판단이 된다.** 여기서 쓰는 동사는 시스템이 한 일 하나(`알렸어요`)다.
 *
 * ★ **없으면 말하지 않는다.** 개입 0인 날에 *"알림은 없었어요"*를 붙이면 잔소리다.
 *   T-44의 *"빈 날에 아무것도 안 쓰기 금지"*와 헷갈리지 않는다: 그건 **요약 전체**의 침묵을 막았고,
 *   이건 **조각 하나**가 없을 때 그 조각만 빼는 것이다. 요약 자체는 여전히 나온다.
 */

/**
 * 문장의 **주어를 사용자로 만드는 표현**. ★ 이 목록의 자리도 여기 하나다(T-44가 세운 규칙).
 *
 * ⚠️ **`CLOSE_JUDGING_WORDS`와 합치지 않는다.** 그쪽은 *어휘*(평가어)를 막고 이쪽은 *주어*를 막는다 —
 * 뜻이 다르고, 합치면 한쪽을 고칠 때 다른 쪽이 딸려 간다. 평가어가 하나도 없는
 * *"어젯밤 늦게 주무셨네요"*를 잡는 것이 이 목록이고, 그것이 둘을 가르는 이유 전부다.
 *
 * 셋으로 나뉜다: ① 사용자를 높이는 어미(`셨`·`시네요`) ② 사용자 지칭 ③ 사용자의 상태·행동 서술.
 */
const CLOSE_USER_SUBJECT_MARKS = [
  "셨", "시네요", "계세", "계셨", "하시", "주무시",
  "당신", "본인", "너는", "님은", "너무 늦",
  "깨어", "주무", "잠들", "잠자", "안 잤", "안 자", "일어나", "늦잠",
  "무시", "답을 안", "반응이 없", "응답이 없", "듣지 않", "지나쳤",
];

/** 발동 시각을 사람 말로. **`Date`를 거치지 않는다** — `fired_at`은 이미 로컬 오프셋 ISO라
 *  문자열에서 시(hour)를 그대로 읽는 것이 기기 시간대와 무관하게 옳다(T-41의 정규화 덕이다). */
function guardHourPhrase(iso) {
  const h = +String(iso).slice(11, 13);
  if (!Number.isFinite(h) || h < 0 || h > 23) return "";
  const label = h === 0 || h === 23 ? "밤" : h < 6 ? "새벽" : h < 12 ? "아침" : h < 18 ? "낮" : "저녁";
  return `${label} ${h % 12 === 0 ? 12 : h % 12}시`;
}

/** 개입이 0이거나 집계를 못 읽었으면 **빈 문자열** — 붙일 말이 없는 것이 정상이다(결정 ②). */
function guardFragment(g) {
  if (!g || !g.fired) return "";
  const n = g.fired;
  const count = n === 1 ? "한 번" : n === 2 ? "두 번" : n === 3 ? "세 번" : `${n}번`;
  const when = g.last_at ? guardHourPhrase(g.last_at) : "";
  return when ? `${when}에 ${count} 알렸어요.` : `${count} 알렸어요.`;
}
